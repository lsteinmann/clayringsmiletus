...
knitr::opts_chunk$set(warning = FALSE, echo = TRUE, fig.width = 12)
...
> *Abstract*: During its excavation in the 1970s, a large number of unidentifiable objects were found in the sanctuary of Dionysos in Miletus: »Peculiar and as of now not explicable for the editor [Willi Real] are numerous fragments of flat rings […]. They are reminiscent of the rings used for modern coal stoves. […] Hitherto no interpretation has been found.« [@real_milet_1977 105]. In the course of a re-examination of the excavation’s findings since 2017, it has been possible to find similar rings from different places in the Mediterranean. It is plausible that these until now unidentified objects are stacking rings used in potters workshops, an isolated and unique find for 5th century Miletus. In this article, the rings will be compared and classified, followed by an assessment of their functionality as well as a discussion of their context. (Preliminary reports of the excavation: @muller-wiener_milet_1977 and @muller-wiener_milet_1979)

The github repository provides all the data used in said paper. This document contains all the code used to generate the relevant graphs and findings in the paper mentioned. It is not intended as an explanation of methodology -- as this is already provided in the paper itself -- but as additional transparency.
...
```{r display_table_1, echo = FALSE}
library(tidyverse)
StackR_Miletus %>%
  filter(!is.na(Inv)) %>% arrange(Inv) %>%
  kable("html") %>%
...
```{r density_prep, echo = TRUE, message= FALSE, fig.height=3}
library(reshape2)
library(ggridges)
...
```{r add_clus_to_df}
StackR_Miletus$HDBScan_Cluster <- as.factor(hdbcluster$cluster)
StackR_Miletus$membership_prob <- hdbcluster$membership_prob
```
...
ggplot(StackR_Miletus, aes(x=max_DM, y=min_DM, color=HDBScan_Cluster, alpha = membership_prob)) +
  geom_point(aes(shape=StackR_Miletus$shape), position=position_jitter(width=0.03,height=.03), size=3) +
  ...
  theme(panel.background = element_blank(), 
        panel.grid.major = element_line(colour = "grey", size = 0.2, linetype = 2),
        axis.text.x = element_text(angle = 45))
```

* Cluster A: `r nrow(StackR_Miletus[which(StackR_Miletus$HDBScan_Cluster == 1),])` rings
* Cluster B: `r nrow(StackR_Miletus[which(StackR_Miletus$HDBScan_Cluster == 2),])` rings
* Cluster C: `r nrow(StackR_Miletus[which(StackR_Miletus$HDBScan_Cluster == 3),])` rings
* noise: `r nrow(StackR_Miletus[which(StackR_Miletus$HDBScan_Cluster == 0),])` rings
...
colnames(datastructure) <- c("height", "min diameter", "max diameter", "width", "thickness") 
rownames(datastructure) <- c("range", "mean", "sd", "cv")

numvar <- 7:11
i = 1
for (col in numvar) {
  datastructure[1,i] <- range(StackR_Miletus[,col])[2] - range(StackR_Miletus[,col])[1]
  datastructure[2,i] <- mean(StackR_Miletus[,col])
  datastructure[3,i] <- sd(StackR_Miletus[,col])
  datastructure[4,i] <- sd(StackR_Miletus[,col]) / mean(StackR_Miletus[,col])*100
  i <- i + 1
}
```
...
tests_dummy <- data.frame(matrix(ncol = 5, nrow = 3000))
colnames(tests_dummy) <- colnames(tests_compare)
library(cluster)
for (row in 1:nrow(tests_dummy)) {
  testsample <- data.frame(matrix(nrow = 67, ncol = 2))
  testsample[,2] <- rnorm(n = 67, mean = datastructure$`min diameter`[2], 
                          sd = datastructure$`min diameter`[3])
  testsample[,1] <- rnorm(n = 67, mean = datastructure$width[2], 
  ...
  sil_hdbscan <- silhouette(test_hdbcluster$cluster, test_dist, do.clus.stat=T, do.n.k=T)
...
tests_compare <- tests_dummy[sample(c(1:nrow(tests_dummy)), replace = FALSE, size = nrow(tests_compare)),]
```
...
sil_hdbscan <- silhouette(hdbcluster$cluster, dist, do.clus.stat=T, do.n.k=T)
```
...
Results from the Silhouette-Test for the original data will be are compiled in a separate dataframe:
```{r }
miletus_data_sil <- data.frame(matrix(ncol = 5, nrow = 1))
colnames(miletus_data_sil) <- colnames(tests_compare)
miletus_data_sil$N[1] <- nrow(StackR_Miletus)
...
p1 <- ggplot(data = tests_compare) +
  geom_density(aes(x = n_cluster, y = ..density..), fill = 'grey') +
...
p2 <- ggplot(data = tests_compare) +
  geom_density(aes(x = n_noise, y = ..density..), fill = 'grey') +
...
p3 <- ggplot(data = tests_compare) +
  geom_density(aes(x = avg_sil_width, y = ..density..), fill = 'grey') +
...
p4 <- ggplot(data = tests_compare) +
  geom_density(aes(x = avg_sil_width_without_0, y = ..density..), fill = 'grey') +
